...
public class MyOpenHelper extends SQLiteOpenHelper implements IOpenHelper{
     ...
     */
    @Override
    public <T> T queryChapter_id(Class<T> table, Object Chapter_id) {
        if (!isTableExists(table)) {
            return null;
        }
        SQLiteDatabase db = getReadableDatabase();
        Cursor cursor = db.query(table.getName().replaceAll("\\.", "_"), null, "Chapter_id" + " = ?", new String[]{String.valueOf(Chapter_id)}, null, null, null);
        List<T> result = initList(table, cursor);
        if (result.isEmpty()) {
            return null;
        } else {
            return result.get(0);//这里是通过Chapter_id查询数据，因为id唯一，所以查到的数据最多也就一个，直接返回list.get(0)
        }
    }


    public <T> T queryChapter_ids(Class<T> table, Object Chapter_ids) {
        if (!isTableExists(table)) {
            return null;
        }
        SQLiteDatabase db = getReadableDatabase();
        Cursor cursor = db.query(table.getName().replaceAll("\\.", "_"), null, "Chapter_ids" + " = ?", new String[]{String.valueOf(Chapter_ids)}, null, null, null);
        List<T> result = initList(table, cursor);
        if (result.isEmpty()) {
            return null;
        } else {
    ...
    //通过orderno查询数据库
    public <T> T queryOrderno(Class<T> table, Object TITLE_NUM) {
        if (!isTableExists(table)) {
            return null;
        }
        SQLiteDatabase db = getReadableDatabase();
        Cursor cursor = db.query(table.getName().replaceAll("\\.", "_"), null, "orderno" + " = ?", new String[]{String.valueOf(TITLE_NUM)}, null, null, null);
        List<T> result = initList(table, cursor);
        if (result.isEmpty()) {
            return null;
        } else {
    ...
    /**
     * 主要删除操作，主要是通过自定义id来删除，因为删除一条操作必须有一个唯一列项
     * @paramobj指定对象（表）中的数据
     * @paramdb
     */

    @Override
    public void delete_id(Class table, int id){
        SQLiteDatabase db = getWritableDatabase();
        db.delete(table.getName().replaceAll("\\.", "_"),"id = ?",new String[]{Integer.toString(id)});
    }

    public void delete_TITLE_NUM(Class table, int TITLE_NUM){
        SQLiteDatabase db = getWritableDatabase();
        db.delete(table.getName().replaceAll("\\.", "_"),"TITLE_NUM = ?",new String[]{Integer.toString(TITLE_NUM)});
    }

    //通过orderno删除数据
    public void delete_Orderno(Class table, int TITLE_NUM){
        SQLiteDatabase db = getWritableDatabase();
        db.delete(table.getName().replaceAll("\\.", "_"),"orderno = ?",new String[]{Integer.toString(TITLE_NUM)});
    }
     ...
     * @return
     */
    public <T> List<T> initList(Class<T> table, Cursor cursor) {
     ...
     * @return
     */
    public Field getFieldId(Class table) {
        Field fieldId = null;
        try {
